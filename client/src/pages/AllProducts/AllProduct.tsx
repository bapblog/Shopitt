...
//   const [products, setProducts] = useState<ProductsProps[]>();
  ...
  const { products,resultPerPage, productsCount } = useAppSelector(
    (state) => state.product
  );
  // filter toggles
  ...
  const priceHandler = (e: Event, newPrice: any) => {
    setPrice(newPrice);
  };
  ...
  }, [data, dispatch, enqueueSnackbar, isError]);
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col gap-2 pb-4 justify-center items-center w-full overflow-hidden bg-white">
                <div className="grid grid-cols-1 sm:grid-cols-4 w-full place-content-start overflow-hidden pb-4 border-b">
                                    {products?.slice((currentPage - 1)*resultPerPage,currentPage* resultPerPage).map((product) => (
                                            <Product {...product} key={product.id} />
                                        ))
                                    }
                                    {/* products.slice(currentpage -1,resultpage + curpage - 1) */}
                                </div>
                {productsCount > resultPerPage && (
                  <Pagination
                    count={Number(
                      ((productsCount + 6) / resultPerPage).toFixed()
                    )}
                    page={currentPage}
                    onChange={(e, val: number) => setCurrentPage(val)}
                    color="primary"
                  />
                )}
              </div>
